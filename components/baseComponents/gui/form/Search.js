...
const Search = React.forwardRef(({options, filter, className, ...rest}, ref) => {
  ...
  const filteredOptions = options.filter(
    filter ??
      (({label}) =>
        text ? label.toLowerCase().search(text.replace(new RegExp("\\\\", "g"), "\\\\").toLowerCase()) !== -1 : true),
  );
  ...
  useEffect(() => {
    ...
    function onDown(e) {
      if (!container.current.contains(e.target)) setOpened(false);
    }
  }, []);

  return (
    <div
      ref={combineRefs([ref, container])}
    ...
    >
      ...
      <div className={"search__list"}>
        <div className={"search__list-block"}>
          <Scrollbar style={{height: "100%"}} className={"scroll"}>
