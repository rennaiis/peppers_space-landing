...
export default function AjaxForm({children, adapter, url, method, onErrorMessage, onResponse, ...rest}) {
  ...
  useEffect(() => {
    if (typeof onErrorMessage === "function") onErrorMessage(errorMessage);
  ...
  function validateResponse(response) {
    if (response.success || response.success === undefined) setResponse(response);
    else if (response.data) {
      if (typeof response.data.message === "string") setErrorMessage(response.data.message);
      else
        Object.entries(response.data.message).forEach(([key, message]) =>
          ref.current.setError(key, {
  ...
  async function onSubmit(data) {
    try {
      ...
      const {response} = e;
      if (response && response.data) validateResponse(response.data);
      console.error(e);
  ...
  return (
    <Form onSubmit={onSubmit} onWatch={errorMessageCallback} ref={ref} {...rest}>
      {children}
    </Form>
  );
}
