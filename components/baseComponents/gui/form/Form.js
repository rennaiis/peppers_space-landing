...
const renderMultipleErrors = ({message, messages}) => {
  ...
  return messages && Object.entries(messages).map(([type, message]) => <Fragment key={type}>{message}</Fragment>);
};
...
const Form = forwardRef(
  (
    ...
    function initInput(child) {
      ...
      const errorData = externalErrors?.errors?.[child.props.name] ? externalErrors.errors : errors;

      return React.createElement(child.type, {
        ...child.props,
        onChange: e => {
          externalErrors?.clearError?.(child.props.name);
          if (typeof child.props.onChange === "function") child.props.onChange.call(this, e);
        },
        control,
        register,
        ...
        error: isLocalErrors
          ? child.props.error || (
              <ErrorMessage name={child.props.name} errors={errorData} render={renderMultipleErrors} />
            )
          : null,
      });
    }

    if (isGlobalErrors) newChildren = addGlobalErrors(newChildren, errors, externalErrors.errors);
    ...
    return React.isValidElement(as) ? React.cloneElement(as, props) : React.createElement(as, props);
...
function addGlobalErrors(children, errors, externalErrors) {
  const errorsComponents = children
    ...
    .map(
      ({props: {error, name}}) =>
        error || (
          <ErrorMessage name={name} errors={{...errors, ...externalErrors}} key={name} render={renderMultipleErrors} />
        ),
    );
