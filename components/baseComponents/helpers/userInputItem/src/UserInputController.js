...
export default class UserInputController {
  static get instance() {
    if (!this._instance) this._instance = new UserInputController();
  ...
  init({ignoreItem, ref, items, queryItems, ...rest}) {
    ...
    if (!ignoreItem) addingItems.push({...rest, ref, componentID});

    if (Array.isArray(items)) items.map(item => addingItems.push({...item, componentID}));
    if (typeof queryItems === "string") {
      ref.current.querySelectorAll(queryItems).forEach(element => {
  ...
  destroy(componentID) {
    const {items} = this;
    items.forEach(item => {
      if (item.componentID === componentID) this.remove(item.uid);
    });
  }
  ...
  registerPlugin(name, item) {
    ...
    subPlugins &&
      subPlugins.forEach(pluginName => {
        this.registerPlugin(pluginName, item);
        this.getPluginByName(pluginName).subscribe(item);
      });

    if (!this.getPluginByName(name)) this.plugins.push(new Cls({items: this.items}));
