...
export default class TransformPlugin extends Plugin {
  ...
  updateItemData(item) {
    ...
    Object.entries(item.transform).forEach(([key, value]) => {
      ...
      isChanged = true;
      if (value.x) tX.push(value.x);
      if (value.y) tY.push(value.y);
      if (value.z) tZ.push(value.z);
    ...
    item.settings.ref.current.style.transform = `translate3d(${item.transform.tx}, ${item.transform.ty},${item.transform.tz})`;
  }
