...
const Sequence = React.forwardRef(
   ...
   * @constructor
   */
  function Sequence({src, keyframesX, keyframesY, totalFrames, paused = false, loop = true, fps = 60, ...etc}, ref) {
    ...
    useEffect(() => {
      ...
      function update() {
        ...
        if (loop || currentFrame !== totalFrames - 1) currentFrame = (currentFrame + 1) % totalFrames;
    ...
    return (
      <div
        className={classNames("sequence", etc.className)}
        ref={ref}
        style={{
