...
export default class AnimationWrapper extends PixiWrapper {
  ...
  static fromCache(name) {
    if (this.cache[name]?.length) return this.cache[name].splice(0, 1)[0];
  }

  static getInstance(uuid) {
    if (!this._instances[uuid]) this.createInstance(uuid);
  ...
  clear() {
    this.stage.children.forEach(child => {
      if (child.isMoveToCache) AnimationWrapper.cacheObject(child, child.name);
    });
  ...
  initChildren(children, data) {
    children.forEach(childData => {
      ...
      if (typeof creator === "function") creator.call(this, childData, data);
    });
  }

  initAnimation(
    {animationContainer, postprocessing, label, frame, type, clip: clipName, transform, useCache = true},
    data,
  ) {
    ...
    if (label) PIXI.animate.Animator.play(clip, label, () => this.stage.emit("animation-controller:animation-end"));
