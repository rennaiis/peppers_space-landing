...
function SequentialWrapper({
  ...
  const memoData = useMemo(
    () => ({
      ...
      callbackOnImageLoad(data) {
        if (!this.isUnmounted) setImagesData(prevState => [...prevState, data].sort((a, b) => a.id - b.id));
      },
      onUnmount() {
  ...
  const image = useMemo(() => {
    const url = getName(1);
    SequenceStorage.instance.loadByName({url, id: 1}).then(data => memoData.callbackOnImageLoad(data));
  ...
  useEffect(() => {
    for (let i = 2; i <= totalFrames; i++)
      SequenceStorage.instance.loadByName({url: getName(i), id: i}).then(data => memoData.callbackOnImageLoad(data));
  ...
  return imagesData.length === totalFrames || parallelLoading ? (
    <SequentialAnimation
      width={width}
      height={height}
