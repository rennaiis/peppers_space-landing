...
module.exports = async function templates({name: componentName, componentType, type = "component"}) {
  ...
  return Promise.all(
    dir
      ...
      .map(async fileName => {
        try {
          const content = await fs.readFile(path.resolve(dirName, fileName), "utf8");
...
function getDirList(dirName, prefix = "") {
  return glob(`**/*`, {cwd: dirName, nodir: true});
  // return new Promise((resolve, reject) =>
  //   glob(`**/*`, {cwd: dirName, nodir: true}, (err, data) => (err ? reject(err) : resolve(data)))
  // );
}
