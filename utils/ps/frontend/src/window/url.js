...
class URL {
  ...
  static parseQueryString(q) {
    return q
      ? q
          ...
          .reduce((result, itm) => {
            itm = itm.split("=");
            result[decodeURI(itm.shift())] = itm.length ? decodeURI(itm.join("=")) : true;
            return result;
          }, {})
      : {};
  }
    ...
    const str = Object.keys(obj)
      .map(
        key =>
          ...
          (obj[key] === true
            ? ""
            : "=" + encodeURI(obj[key]).replace(/[!"()*]/g, c => `%${c.charCodeAt(0).toString(16)}`)),
      )
      .join(sep);
  ...
  extend(url) {
    ...
    ["protocol", "host", "port", "path", "query", "hash"].forEach(key => {
      if (key !== "query") {
