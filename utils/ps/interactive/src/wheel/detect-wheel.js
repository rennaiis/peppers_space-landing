function initMouseWheel({
  ...
  function onWheel(e, isEmulation) {
    ...
    let _deltaY = typeof e.deltaY === "undefined" ? -e.wheelDelta : e.deltaY;
    ...
    if (max_deltaY === 0) {
      ...
      if (_deltaY_abs > prev_deltaY_abs + 3 && _time - max_deltaY_time > checkTimeout) {
        // ... наростания в случае повторного скролла но не чаще 200мсек...
        max_deltaY = 0;
      }

      clearTimeout(wait_for_deacy_timeout);
      wait_for_deacy_timeout = setTimeout(onWaitForDecay, maxCompleteTimeoutDelay);
    }
    // }
    ...
    if (typeof onBeforeCheck == "function") onBeforeCheck(e, current_direction);
    ...
    if (typeof onAfterCheck == "function") onAfterCheck(e, current_direction);
  }
  ...
  function onWaitForDecay() {
    ...
    max_deltaY = 0;
    prev_direction = 0;
    if (typeof onWaitForDecayCallback === "function") onWaitForDecayCallback();
  }
