...
class Builder {
  ...
  createExtraReducer({thunkName, thunkExtraName, func, onSubmit, saveData, saveError, controller, ...etc}) {
  ...
  createSelector(name, selector, equalityFn) {
    this.selectors[camelCase(`use-${name}`)] = function useSomeSelector() {
      return useSelector(state => selector.call(null, state, ...arguments), equalityFn);
    };
    return this;
  }
  ...
  create() {
    ...
    const slice = createSlice({
      ...
      extraReducers: builder => {
        Object.entries(extraReducers).forEach(([actionName, reducer]) => builder.addCase(actionName, reducer));
