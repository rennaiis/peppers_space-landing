...
t.goodWords = ["дезмонда", "застрахуйте", "одномандатный", "подстрахуй", "психуй"];
...
t.containsMat = function (text) {
  ...
  for (let i = 0; i < words.length; i++) {
    ...
    if (t.isInGoodWords(word) && t.isInGoodPatterns(word)) continue;

    if (t.isInBadPatterns(word)) return true;
  }

  if (t.containsMatInSpaceWords(words)) return true;
...
t.convertEngToRus = function (word) {
  for (let j = 0; j < word.length; j++) {
    for (let key in t.letters) {
      if (word.charAt(j) === key) word = word.substring(0, j) + t.letters[key] + word.substring(j + 1, word.length);
    }
  }
